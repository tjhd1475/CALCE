...
def load_video_audio(
    ...
    ar = AudioReader(uri=audio_path)

    vlen = len(vr)
    ar = torch.tensor(ar._array[0])
    end_load_aud = time.time()
    alen = len(ar)
    n_frms = min(n_frms, vlen)
    fps = vr.get_avg_fps()
    if clip_proposal is None:
    ...
    # get_batch -> T, H, W, C
    frms = vr.get_batch(indices).permute(3, 0, 1, 2).float()  # (C, T, H, W)
    end_pre_vid = time.time()
    audio_new_len = (alen+n_frms)// n_frms * n_frms
    audio_trunk_size = audio_new_len // n_frms
    if alen % audio_trunk_size > 0:
        padding_size = audio_trunk_size - alen % audio_trunk_size
        ar = torch.cat([ar,torch.zeros(padding_size)])
    else:
        padding_size = 0
    ar = ar.reshape(n_frms,-1).float()
    aud_padding_mask = torch.zeros_like(ar)
    aud_padding_mask[-1,-padding_size:] = 1
    aud_padding_mask = aud_padding_mask.bool()
    end_pre_aud = time.time()
    # print(f"load vid:{end_load_vid-start_time:.2f}\tload aud:{end_load_aud-end_load_vid:.2f}\t"
          # f"pre vid:{end_pre_vid-end_load_aud:.2f}\t pre aud:{end_pre_aud-end_pre_vid:.2f}")
    return frms, ar, aud_padding_mask, indices, fps
