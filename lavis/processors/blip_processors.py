...
from lavis.datasets.data_utils import load_video, load_video_audio
...
@registry.register_processor("blip2_va_train")
class Blip2VidAudTrainProcessor(BlipVideoBaseProcessor):
    def __init__(
        self,
        image_size=384,
        mean=None,
    ...
    def __call__(self, vpath, apath, clip_proposal=None):

        clip, audio,aud_padding_mask, indices, fps = load_video_audio(
            video_path=vpath,
            audio_path=apath,
            n_frms=self.n_frms,
            height=self.image_size,
        ...
        return self.transform(clip),audio,aud_padding_mask, indices, fps
