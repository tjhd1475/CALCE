...
class MomentRetrievalTask(BaseTask):
    ...
    def setup_task(cls, cfg):
        return cls(cfg.run_cfg.debug)

    def valid_step(self, model, samples):
        outputs = model.generate(samples)
        bs = len(next(iter(outputs.values())))

        results = [{key: outputs[key][i] for key in outputs} for i in range(bs)]
    ...
    def after_evaluation(self, val_result, split_name, epoch, **kwargs):
        ...
        if is_main_process():
            metric_file = os.path.join(
                result_dir, "{}_epoch{}_metric".format(split_name, epoch)
            )
            json.dump(metrics, open(metric_file,'w'))
            if wandb.run is not None:
                wandb.log(
                    {
                        "eval/agg_metrics(r1_avg)": metrics["agg_metrics"],
    ...
    def _report_metrics(self, eval_result_file, split_name):
        ...
        results_interpreted = [
            {
                "qid": r["qid"],
                "pred_relevant_windows": r["pred_windows"],
                "relevant_windows": self.moment_str_to_list(r["gt_windows"]),
            }
            for r in results
        ]

        all_metrics = eval_submission(results_interpreted)
        ...
        metrics = {
            "agg_metrics": r1_avg,
            "r1": all_metrics["mr"]["full"]["R1"],
            "mAP": all_metrics["mr"]["full"]["mAP"],
            "mIoU": mIoU,
    ...
    def _train_inner_loop(
        ...
        for i in metric_logger.log_every(range(iters_per_epoch), log_freq, header):
            ...
            loss_collect.append({'loss':loss.item(),'qid': samples['query_id']})
        ...
        result['loss_collect'] = loss_collect
        return result
