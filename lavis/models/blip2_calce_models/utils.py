...
def post_process(pred,duration=None):
    ...
    """
    pred = pred.split("</s>")[0]
    if duration is not None and isinstance(duration,torch.Tensor):
        duration = int(duration)
    # check if the string has the right format of a nested list
    # the list should look like this: [[0, 1], [4, 7], ...]
    # if not, return "[[-1, -1]]"
    window_list = [[-1, -1]] if duration is None else [[0, duration]]
    ...
    def add(start, end):
        ...
        if duration is not None and end > duration:
            if str(duration) in str(end):
                end = int(str(end).replace(str(duration), ''))
            elif len(str(end)) == 4:
                end = end % 100
            elif len(str(end)) == 6:
                end = end % 1000
    ...
    for window in windows:
        ...
        # if no number or more than two number, ignore this window
        # e.g.
        #   "[,]"
        if len(numbers) == 0:
            continue
        if len(numbers) > 2:
            numbers = numbers[:2]
        # if only one number, save it for two number
        # e.g.
        #   "[1,]" → save 1
        if len(numbers) == 1:
            if save is not None:
                t_start, t_end = min(save,int(numbers[0])),max(save,int(numbers[0]))
                add(t_start,t_end)
                save = None
            else:
                save = int(numbers[0])
        ...
        # if two numbers
            ...
            t_start, t_end = int(numbers[0]),int(numbers[1])
            ...
            # reuse save
            # e.g.
            #   "save 1" -> "[1, t_start-1]", save None
            if save is not None and save < t_start:
                add(save,t_start-1)
                save=None
    # reuse save
    # e.g.
    #   "save 1" -> "[1, duration]"
    if save is not None and duration:
        add(save, duration)
    window_list = sorted(window_list, key=lambda x: x[0])
    return window_list
...
def round_item(x):
    ...
    if isinstance(x, torch.Tensor):
        ...
        raise "Not support element type"
...
def format_wandb_log_images_and_predictions(
    ...
    for frame in image[idx]:
        ...
        frames.append(frame)
    if video_prompt is None or text_prompt is None:
        query = ""
    else:
        query = video_prompt[idx] + text_prompt[idx]

    pred = pred[idx]
    duration = samples["duration"][idx]
    processed_pred = post_process_fn(pred,duration=duration)
    qid = qid[idx]
    target = target[idx]
    etc_val = [str(round_item(e)) if isinstance(e,torch.nn.Parameter) else str(round_item(e[idx])) for e in list(etc.values())]
...
def get_cluster_timestamps_as_seconds_integers(cluster_timestamps,annoying_numbers_replacement_dict=None):
    new_timestamps = []
    for cluster_timestamp in cluster_timestamps:
        new_timestamp = []
        for t in cluster_timestamp:
            new_timestamp.append(
                torch.tensor(
                    [
                        int(
...
def get_timestamps_as_seconds_integers(
    ...
    for t, duration in zip(timestamps, durations):
        if isinstance(t, torch.Tensor):
            t = t.tolist()
        # convert to absolute timestamps
        _video_prompt = ">".join(
            (
                str(int(round(timestamp)))
                if round(timestamp)
                not in annoying_numbers_replacement_dict.keys()
                else str(annoying_numbers_replacement_dict[round(timestamp)])
            )
            for timestamp in t
        )
            ...
            torch.tensor(
                [
                    int(
                        ...
                        if round(timestamp)
                        not in annoying_numbers_replacement_dict.keys()
                        else annoying_numbers_replacement_dict[round(timestamp)]
                    )
                    for timestamp in t
                ]
            )
