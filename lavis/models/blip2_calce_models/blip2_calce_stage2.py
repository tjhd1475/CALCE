...
@registry.register_model("blip2_calce_stage2")
class BLIP2_CALCE_STAGE2(Blip2Base):
    """
    BLIP2 T5 model.
    ...
    def __init__(
        ...
        self.log_samples_every_n = 1500
        self.log_samples_every_n_eval = 600
    ...
    def forward(
        ...
        _, n, c = video_embeds.shape
        # video_embeds = video_embeds.reshape(b,t,n,c)   # b, t, n, c
        ...
        if self.input_time_format == 'frame_indices':
            ...
            timestamps, relevant_windows,batch_stage1_pred_windows, durations =self.absolute_times_to_frame_indices(timestamps, relevant_windows, batch_stage1_pred_windows, durations, amount=self.format_target_amount)
        ...
        cluster_features = F.normalize(frames_for_projection, dim=-1)  # b,t, n_query
        cluster_features = cluster_features.reshape(b,t,-1).detach()    # b, t, n_query*d
        ...
        topk = 8
        cluster_filter_results = []
        for stage1_pred_windows, cluster_idx, timestamp in zip(batch_stage1_pred_windows, batch_cluster_idx, timestamps):
            stage1_pred_windows_flatten = torch.zeros(t, dtype=torch.int, device=device)
            cluster_cnt = torch.zeros(self.num_cluster_center,device=device)
            for window in stage1_pred_windows:
                start, end = window
                start_idx = [index for index, ts in enumerate(timestamp) if round(ts)>= start][0]
                end_idx = [index for index, ts in enumerate(timestamp) if round(ts) <= end][-1]
                stage1_pred_windows_flatten[start_idx:end_idx] = 1

                for pos in range(start_idx, end_idx):
                    cluster_cnt[cluster_idx[pos]] += 1

            topk_clusters = torch.argsort(cluster_cnt)[-topk:]
            topk_clusters = topk_clusters[torch.where(cluster_cnt[topk_clusters] != 0)]
            cluster_filter_result = torch.isin(cluster_idx, topk_clusters).to(torch.int)    # t
            cluster_filter_result = cluster_filter_result | stage1_pred_windows_flatten
            cluster_filter_result = self.two_side_bias_padding(cluster_filter_result,bias=self.bias2)
            cluster_filter_results.append(cluster_filter_result)
            # cluster_filter_result = self.compress_mask(cluster_filter_result)   # sum(cluster_filter_result) = t'

        cluster_filter_results = torch.stack(cluster_filter_results)
        # frame_embeds_filtered = torch.cat(frame_embeds_filtered)    # b, t', n_query, c
        # cluster_features = F.normalize(frame_embeds_filtered, dim=-1)  # b, t', n_query, c
        # cluster_features = cluster_features.reshape(b, frame_embeds_filtered.shape[1], -1).detach()  # b, t' , n_query*c
        ...
        for cluster_idx,cluster_filter_result, subtitle_durations,duration,q in (
                zip(batch_cluster_idx,cluster_filter_results, batch_subtitle_durations,durations,query)):
            # native key frame (cluster edges)
            cluster_idx[cluster_filter_result==0] = -1
            key_frame_mask = torch.zeros(t, dtype=torch.int,device=device)
            for i in range(1, t):
            ...
            if self.is_talk_relative(q) and subtitle_durations:
                ...
                for subtitle_duration in subtitle_durations:
                    ...
                    for i in range(start,end+1):
                        key_frame_mask[i] = 1
            key_frame_mask = self.two_side_bias_padding(key_frame_mask,bias=self.bias1)
            key_frame_mask = key_frame_mask & cluster_filter_result
            non_key_frame_mask = (1 - key_frame_mask) & cluster_filter_result
            key_frame_mask, non_key_frame_mask = self.compress_mask(key_frame_mask,non_key_frame_mask,target_count=t//2)
        ...
        batch_frames_filtered,batch_frames_atts,batch_timestamps_filtered = [], [], [] # b, t, n_query or n_query/combine_ratio, c; b, t, n_query or n_query/combine_ratio
        for key_frame_idx, non_key_frame_idx, frames, timestamp in zip(batch_key_frame_idx,batch_non_key_frame_idx,frames_for_projection,timestamps):
            ...
            frames, frames_atts, timestamp_filtered = [], [], []
            t_n, t_k = 0, 0
            for t_i in range(t):
                if t_k<len(key_frame_idx) and t_i == key_frame_idx[t_k]:
                    ...
                    timestamp_filtered.append(timestamp[t_i])
                elif t_n<len(non_key_frame_idx) and t_i == non_key_frame_idx[t_n]:
                    frames.append(non_key_frames[t_n])
                    t_n += 1
                    frames_atts.append(torch.ones(frames[-1].shape[:-1], dtype=torch.long, device=device))
                    timestamp_filtered.append(timestamp[t_i])

            assert len(frames) == len(timestamp_filtered)
            batch_frames_filtered.append(frames)
            batch_frames_atts.append(frames_atts)
            batch_timestamps_filtered.append(torch.tensor(timestamp_filtered))
        ...
        with torch.cuda.amp.autocast(dtype=torch.bfloat16):
            ...
            inputs_embs, inputs_atts, video_prompt, text_prompt, subtitle_prompt, embed_lens = self.mr_prompt_concatenation(
                batch_timestamps_filtered, durations, batch_frames_filtered,  batch_subtitles, query, task_prompt,
                item_prompt=item_prompt, stage1_prompt=batch_stage1_pred_windows, device=device)
        ...
        result_str = [f'{_qid}: \t s1: {s1}\traw_pred: {pw}\tpred: {pwp}\t gt: {rw}' for _qid, s1, pw, pwp, rw in
                      zip(qid, batch_stage1_pred_windows, pred_windows, pred_windows_processed, relevant_windows)]
    ...
    def compress_mask(self,key_mask,non_key_mask,target_count=75):   # todo
        def _get_mask_clusters(mask):
            clusters = []
            current_cluster = []
        ...
        max_tries,now_tries = 1000,0
        while torch.sum(key_mask)+torch.sum(non_key_mask) > target_count and now_tries < max_tries:
            now_tries +=1
            non_key_clusters = _get_mask_clusters(non_key_mask)
            if non_key_clusters:
                non_key_largest_cluster = max(non_key_clusters, key=len)    # 找到最大的簇
                new_non_key_mask = non_key_mask.clone()
                new_non_key_mask[non_key_largest_cluster] = 0
                new_non_key_mask[non_key_largest_cluster[::2]] = 1  # 取间隔一个的元素
                non_key_mask = new_non_key_mask
                if torch.sum(key_mask)+torch.sum(non_key_mask) <= target_count:
                    break
            key_clusters = _get_mask_clusters(key_mask)
            if not key_clusters:
                break
            key_largest_cluster = max(key_clusters, key=len)
            new_key_mask = key_mask.clone()
            new_key_mask[key_largest_cluster] = 0
            new_key_mask[key_largest_cluster[::2]] = 1  # 取间隔一个的元素
            key_mask = new_key_mask

        pk,pn = 0.3, 0.7
        def _rand_p_zero(mask,p):
            # 找到所有1的位置
            indices = torch.nonzero(mask == 1)
            # 随机选择一个位置
            if indices.numel() > 0:
                random_index = indices[torch.randint(0, indices.size(0), (1,)).item()]

                # 以概率 p 将该位置的值设置为0
                if torch.rand(1).item() < p:
                    mask[random_index] = 0

        while torch.sum(key_mask) + torch.sum(non_key_mask) > target_count:
            _rand_p_zero(non_key_mask,pn)
            _rand_p_zero(key_mask,pk)

        return key_mask, non_key_mask
    ...
    def mr_prompt_concatenation(
        ...
        if stage1_prompt and item_prompt:
            stage1_prompt = ['ref: '+str(s1)+ "\n" for s1 in stage1_prompt]
            text_prompt = [i + s1+ it + t for i, s1, it, t in zip(query_prompt, stage1_prompt, item_prompt, task_prompt)]
        elif item_prompt:
    ...
    def generate(
        ...
        batch_frames_filtered, batch_frames_atts, batch_timestamps_filtered = [], [], []  # b, t, n_query or n_query/combine_ratio, c; b, t, n_query or n_query/combine_ratio
        for key_frame_idx, non_key_frame_idx, frames, timestamp in zip(batch_key_frame_idx, batch_non_key_frame_idx,
                                                                       frames_for_projection, timestamps):
    ...
    def from_config(cls, cfg):
        ...
        bias1 = cfg.get("bias1", 3)
        bias2 = cfg.get("bias2", 1)
    ...
    def absolute_times_to_frame_indices(self, batch_absolute_times, batch_absolute_time_relevant_windows,batch_stage1_pred_windows, batch_duration, amount=120):
        batch_frame_indices = []
        batch_frame_index_relevant_windows = []
        batch_frame_index_stage1_pred_windows = []

        for absolute_times, absolute_time_relevant_windows,stage1_pred_windows,  duration in zip(batch_absolute_times,batch_absolute_time_relevant_windows,batch_stage1_pred_windows,batch_duration):
            ...
            frame_index_stage1_pred_windows = [[round(t * amount / duration) for t in w] for w in stage1_pred_windows]
            batch_frame_index_relevant_windows.append(str(frame_index_relevant_windows))
            batch_frame_index_stage1_pred_windows.append(frame_index_stage1_pred_windows)
        ...
        return batch_frame_indices,batch_frame_index_relevant_windows, batch_frame_index_stage1_pred_windows, batch_duration

    def frame_indices_to_absolute_times(self, batch_frame_indices, batch_frame_index_pred_windows, batch_duration, amount=120):
